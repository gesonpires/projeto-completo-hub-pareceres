...
export default async function AtosPage({
  ...
  try {
    const orderBy = (() => {
      switch (sort) {
        case "instituicao":
          return [{ instituicao: { nomeNormalizado: dir } }, { dataAto: "desc" as const }];
        case "tipo":
          return [{ tipo: dir }, { dataAto: "desc" as const }];
        case "numero":
          return [{ numero: dir }, { dataAto: "desc" as const }];
        case "dataAto":
        default:
          return [{ dataAto: dir }];
      }
    ...
    [total, atos] = await withPrismaRetry(() =>
      Promise.all([
        prisma.atoAutorizativo.count({ where }),
    ...
    <div className="flex flex-1 flex-col bg-zinc-50">
      ...
      <div className="mx-auto w-full max-w-5xl flex-1 px-6 py-10">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-lg font-semibold tracking-tight">Atos</h1>
          ...
          <div className="flex items-end gap-2 sm:col-span-2">
            <div className="w-full">
              ...
              <select
                id="sort"
                name="sort"
                defaultValue={sort || "dataAto"}
                className="mt-1 h-10 w-full rounded-md border border-zinc-200 bg-white px-3 text-xs text-zinc-900 outline-none focus:border-zinc-400"
              >
                <option value="dataAto">Data</option>
        ...
        {dbError ? (
          <ErrorAlert message={dbError} dismissHref={`/atos?${queryBase.toString()}`} className="mt-6" />
        ) : null}
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-zinc-200 bg-white">
                ...
                >
                  <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
                    ...
                    <div className="text-xs text-zinc-600">
                      {a.instituicao.cnpj ? `CNPJ ${formatCnpj(a.instituicao.cnpj)}` : "CNPJ não informado"}
                      {" • "}
                      {a.instituicao.municipio ? a.instituicao.municipio : "Município não informado"}
                      {a.instituicao.uf ? `/${a.instituicao.uf}` : ""}
                    </div>
                  </div>
                  <div className="mt-1 text-xs text-zinc-700">
                    <span className="font-medium">{a.instituicao.nome}</span>
                    {a.ementa ? <span className="text-zinc-500"> • {a.ementa}</span> : null}
                  </div>
                </Link>
              </li>
            ))}
          ...
          <div className="flex items-center justify-between border-t border-zinc-200 px-4 py-3 text-xs">
            <div className="text-zinc-600">Mostrando {atos.length} de {total}</div>
